...
def test_set_get():
    ...
    # To Check if the ValueError message contains the expected text
    expected_message = "Negative values are not allowed"
...
def test_recommend():
    ...
    assert matrix.recommend(user_vt) == [1, 0, 0]

    # Test recommending with an invalid vector, for the dimensions not matching
    with pytest.raises(ValueError):
        matrix.recommend([1, 0])

def test_add_movie():

    # Initialize the two SparseMatrices of the same dimensions
...
def test_recommend_invalid_vector():
    ...
    with pytest.raises(ValueError):
        matrix.recommend(user_vt)

def test_for_movie_invalid_dimensions():
    # Initialize two SparseMatrices with different dimensions
    matrix1 = SparseMatrix(3, 3)
